...
// no of heap elements would be arrLen - 1 as index 0 is not counted
...
int main()
{
    ...
    int arr[] = {4, 8, 6, 99, 2,33,1};
    initHeapFromArray(&hh,sizeof(int),arr,7,&minHeapComp);
    //printAsInt(hh);
    for(int i=0;i<6;i++)
    {
        printAsInt(hh);
        int buffer;
        extract(hh,(void*)&buffer);
        printf("\nExtractMax: %d\n",buffer);
    }
